...
func generate():
	...
	var rp_vec2 : PackedVector2Array = []
	var del_graph : AStar2D = AStar2D.new()
	var mst_graph : AStar2D = AStar2D.new()
	
	#Initializing room_positions as a vec2, to be used for the delaunay triangulaation, and creating the delaunay and MST graph nodes
	for p in room_positions:
		rp_vec2.append(Vector2(p.x,p.z))
	...
	#Generating a delaunay graph using the builtin function
	var delaunay : Array = Array(Geometry2D.triangulate_delaunay(rp_vec2))
	
	#Connecting the vertices of the delaunay graph together
	...
	while visited_points.size() != mst_graph.get_point_count():
		...
		for pc in possible_connections:
			if rp_vec2[pc[0]].distance_squared_to(rp_vec2[pc[1]])<\
			rp_vec2[connection[0]].distance_squared_to(rp_vec2[connection[1]]):
				connection = pc
...
func create_hallways(hallway_graph : AStar2D):
	...
	for h in hallways:
		...
		for t in hall:
			var pos : Vector3 = Vector3i(t.x,0,t.y)
...
func make_room(rec:int):
	...
	for r in range(-room_margin,height+room_margin):
		for c in range(-room_margin,height+room_margin):
